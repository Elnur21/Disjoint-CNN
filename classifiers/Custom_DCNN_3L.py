...
class Classifier_DCNN_3L:
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        # Pooling layers
        max_pool = MaxPooling1D(pool_size=5, strides=None, padding='valid')(conv3)
    ...
    def fit(self, Ximg_train, yimg_train, Ximg_val, yimg_val, epochs, batch_size):
        if self.verbose:
            print('[DCNN_3L] Training DCNN_3L Classifier')
        ...
        if self.verbose:
            print('[DCNN_3L] Training done!, took {}s'.format(self.duration))
        return

    def predict(self, X_img, y_img, best):
        if self.verbose:
            print('[DCNN_3L] Predicting')
        if best:
